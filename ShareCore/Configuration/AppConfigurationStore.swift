...
public final class AppConfigurationStore: ObservableObject {
  ...
  private func loadConfiguration() {
    ...
    if tryLoadConfiguration(named: "Configuration") {
      print("[ConfigStore] ✅ Loaded configuration from App Group")
    } else {
      print("[ConfigStore] ❌ Failed to load configuration, creating empty")
  ...
  public func resetToDefault() {
    ...
    if let name = currentConfigurationName {
      do {
        ...
        print("[ConfigStore] ⚠️ Failed to delete configuration: \(error)")
      }
    }

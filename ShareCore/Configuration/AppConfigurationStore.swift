...
public final class AppConfigurationStore: ObservableObject {
    ...
    private init() {
        let defaultProvider = Defaults.provider
        self.providers = [defaultProvider]
        self.actions = Defaults.actions(for: defaultProvider.id)
    }
...
private enum Defaults {
    private static let providerURL = URL(
    ...
    static func actions(for providerID: UUID) -> [ActionConfig] {
        [
            .init(
                name: "翻译",
